...
\end{align*}
from which we can obtain the sample size
\begin{align*}
...
\Sexpr{format(nsim, big.mark = "'")} Bayes factor were computed. The power was
...
We can see that in all conditions, the simulation-based estimate of the power is
