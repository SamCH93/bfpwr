...
with dementia. The primary endpoint of the trial was the Cohen-Mansfield
Agitation Inventory (CMAI) at 12 weeks after treatment start, and the treatment
effect was quantified as the difference in mean CMAI between the mirtazapine and
placebo groups $\theta$. The null hypothesis was defined as no difference
between mirtazapine and placebo ($H_{0} \colon \theta = 0$), while the
alternative hypothesis was formulated as a mean decrease of six CMAI points
($H_{1} \colon \theta = \Sexpr{pm}$). With a target power of $1 - \beta =
\Sexpr{pow*100}\%$, a two-sided level of $\alpha = \Sexpr{alpha}$, and an
assumed standard deviation of $\sigma = \Sexpr{sd}$ points,
formula~\eqref{eq:freqsamplesize} gives a required per-group sample size of $n =
\Sexpr{ceiling(n)}$, which the investigators adjusted upwards to 111 to account
for attrition. Contrary to these expectations, however, the trial did not find a
statistically significant effect of mirtazapine over placebo and the 95\%
confidence interval spanned a wide range of parameter values from beneficial to
evene harmful effects (estimated mean difference of \Sexpr{est} with 95\%
confidence interval from \Sexpr{ci[1]} to \Sexpr{ci[2]}, $p = \Sexpr{round(p,
  3)}$). This was especially problematic as also a slightly higher mortality was
observed in the mirtazipine group, leading the investigators to recommend
against using mirtazipine for the treatment of agitation in patients with
dementia.
...
the null hypothesis of no effect over the specified alternative hypothesis,
although this level of evidence is merely anecdotal and far from compelling.
However, if we take a step back and assume that no data have yet been collected,
we can use the equations~\eqref{eq:prLR} and \eqref{eq:nLR} to calculate power
and sample size, and design the study for compelling inferences. We will
...
\caption{Power and sample size calculations for randomized clinical trial
  assessing the effect of the antidepressant mirtazapine on agitated behavior
  compared to placebo in patients with dementia \citep{Banerjee2021}. The Bayes
  factor assumed for the analysis contrasts $H_{0} \colon \theta = \Sexpr{null}$
  to $H_{1} \colon \theta = \Sexpr{pm}$. The power is computed under either a
  point prior at the assumed effect under the alternative (green), a normal
  prior that incorporates additional parameter uncertainty (blue), or under the
  null hypothesis (orange).}
...
Focusing now on the bottom plot, we can see that the same number of events
...
controlled (below the conventional 5\%) for the point design prior (green),
while it is slightly inflated for the normal design prior (blue) after more than
100 events.
