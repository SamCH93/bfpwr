...
\author{
  ...
  {\color{blue} Preprint version March 13, 2024}
}
\date{}
...
% ======================================================================
\begin{abstract}
  An important aspect of study design is the determination of an appropriate
  sample size. The choice of sample size should be consistent with the planned
  analysis. If the planned analysis involves Bayes factor hypothesis testing,
  the sample size is typically desired to ensure a sufficiently high probability
  of obtaining compelling evidence for a hypothesis, given that the hypothesis
  is true. In practice, sample size determination based on Bayes factor analyses
  are typically performed using computationally intensive Monte Carlo simulation
  methods. Here we summarize how, under approximate normality assumptions,
  sample sizes can be determined numerically without simulation, and provide the
  R package \texttt{bfpwr} for doing so. We also identify conditions under which
  sample sizes can be determined in closed-form, leading to novel formulas that
  are easy-to-use, help fostering intuition, and enable asymptotic analysis.
  Case studies from medicine and psychology illustrate how researchers can use
...
should be aligned with the planned analysis, or in other words: `\emph{As ye
  shall analyse is as ye shall design}' \citep[p.~179]{Julious2023}.

A widely used formula for the sample size per group for data with continuous
outcome based on a frequentist hypothesis test of a mean difference is given by
\begin{align}
...
one sample \citep[p.~34]{Matthews2006}. There exist various refinements
of~\eqref{eq:freqsamplesize} that adapt it to special study designs (e.g.,
cross-over studies) or data types (e.g., binary data), see \citet{Julious2023}.
For many analysis methods, however, no closed-form formula exist and iterative
or simulation methods have to be used. Nevertheless, while typically only being
an approximation, the formula~\eqref{eq:freqsamplesize} allows for quick
...
factors \citep{Weiss1997, DeSantis2004, Schoenbrodt2017, Schnbrodt2017b,
  Stefan2022, Pawel2023e}. However, to our knowledge, there are no simple
...
Bayes factors analysis based on numerical root-finding that, while not being
...
The Bayes factor~\eqref{eq:BF01} may be thought of as a `Bayesian $z$-test',
...
assumption is in most applications not true, yet it makes the test widely
...
Furthermore, as we will show, it also allows us to conduct closed-form power and
...
assumed design prior, one may not be able with the power
function~\eqref{eq:prLR} to approach a power of one by increasing the sample
size $n$. That is, the limitting power value is given by
\begin{align}
  \label{eq:powerlimLR}
...
\end{align}
The formula~\eqref{eq:nLR3} was also found by \citet{Strug2007} for `evidential'
sample size calculations, but is unfortunately not well known. Unsurprisingly
the two formulae coincide as Bayes factors and likelihood ratios -- the measure
of evidence used in `evidential statistics' \citep[see e.g.,][]{Royall1997,
  Blume2002, Strug2018, Perneger2021} -- coincide when the Bayes factor involves
only point hypotheses. Our more general formula~\eqref{eq:nLR} thus also
generalizes evidential sample size calculations to incorporate parameter
...
value is commonly done in `default' Bayes factor tests \citep{Berger1987b}. We
then have that $M=0$ and hence the power~\eqref{eq:prBF} reduces to
\begin{align}
  \label{eq:prBFcenter}
...
represents a distribution of effect sizes that are centered around the null
value while the point alternative from~\ref{tab:smdn} represents a mean
difference of one standard deviation away from the null. The former is much more
pessimistic than the latter. To incorporate more optimistic beliefs into the
calculations we may increase the standard deviation of the distribution $\tau$.
For example, doubling the standard deviation $\tau$ leads to a four-fold
decrease of the sample size~\eqref{eq:nBF}.
...
($H_{0} \colon \theta = 0$), while the alternative defined as a clinically
...
the required sample size to $n = \Sexpr{n2}$. Looking at the orange curve in the
bottom plot, we see that these same sample sizes only have modest power of
around $20\%$ and $50\%$, respectively, for obtaining evidence in favor of the
true null hypothesis (at a threshold of $k = 6$). To obtain a power of $80\%$,
say, around $n = 800$ samples per group would be required. This illustrates the
well-known fact that evidence accumulates slower under point null hypotheses
compared to normal alternatives when the Bayes factor involves testing a point
null against a normal alternative \citep{Johnson2010}. Finally, looking at the
orange curve in the top plot and the green/blue curves in the bottom plot, we
see that the probability of misleading evidence in favor of the incorrect
hypothesis appears to be adequately controlled, as these curves are virtually
zero over the entire range of sample sizes.


\section{Discussion}
\label{sec:discussion}
We presented methods for performing power and sample size calculations intended
for settings where future data are analyzed with Bayes factor hypothesis tests.
...
\citep{Wald1947}. For this purpose, it would be interesting to consider the
