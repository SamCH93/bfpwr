...
The Bayes factor~\eqref{eq:BF01} has already appeared before in the literature
in one form or another \citep[e.g., in][]{Weiss1997, DeSantis2004,
  Spiegelhalter2004, Bartos2023b}. It may be thought of as a `Bayesian
\mbox{$z$-test}', that is, a test of a normal mean based on an asymptotically
normal statistic assuming that the variance of the statistic is known. Of
course, the latter assumption is not true in most applications, but it makes the
test widely applicable and is, for practical purposes, often close enough to
Bayes factors based on the exact distribution of the data, which may or may not
be available. Finally, the Bayes factor~\eqref{eq:BF01} can also be used with
parameter estimates where a standard error is available but not of the form
...
perform closed-form power and sample size calculations.
...
factors \citep{Pawel2022b, Wagenmakers2022, Pawel2023}.
...
encodes the assumption that some parameters are larger while others are
