...
sciences then illustrates how our formulas can be implemented by researchers in
...
the design of the study \citep{OHagan2001b, OHagan2005}. This prior should
...
\end{align}
Assuming, for instance, that the tested parameter is a mean difference with unit
variance $\sigma^2_{\scriptscriptstyle \hat{\theta}} = 2\sigma^{2}$ (see
...
Unsurprisingly the two formulas coincide as Bayes factors and likelihood ratios
...
formulas~\eqref{eq:nLR} and~\eqref{eq:nLR2} thus also generalize evidential
sample size calculations to incorporate different assumed design parameters
and/or parameter uncertainty, similar to how Bayesian design priors can be used
for incorporating parameter uncertainty in a planned frequentist analysis
\citep[see e.g.,][for an overview of Bayesian/frequentist hybrid design
...
\caption{Sample size per group $n$ to obtain a Bayes factor
  ...
  ($\mu - \theta_{0} = 1$) so that equation~\eqref{eq:nLR3} can be used to
  compute the sample size in closed-form.}
\label{tab:smdn}
...
potential existence of a sample size that can achieve the target power: Since
the argument of the Lambert W function has to be at least $-1/e$ for it to be
...
\caption{Required unit information sample size $n_{k,\beta}$ computed with
  equation~\eqref{eq:nBF} to obtain a Bayes factor smaller than $k$ with at
  least a power of $1-\beta$ with a unit information analysis and design prior.}
\label{tab:nnormal}
...
\caption{Power and sample size calculation for example adapted from
  ...
  uncertainty (blue), or under the null hypothesis
  (orange). % The design prior is either set
...
The types of Bayes factor that we considered so far are limited to data in the
form of asymptotically normally distributed parameter estimates, and normal or
point priors under the alternative. Researchers may also want to use different
data models or prior distributions. In the following we outline two extensions.
...
\begin{enumerate}
  \item For a given sample size $n$ and analysis prior
        ($\mu, \tau, \kappa, a, b$), use numerical root-finding to determine the
        `success region' $S$ in terms of $t$-statistics where the Bayes factor
        is equal or below the threshold $k$, i.e.,
...
specify a one-sided scaled Cauchy prior ($\mu = 0$, $\kappa = 1$, $a = 0$,
$b = +\infty$) with scale $\tau = 1/\sqrt{2}$ as in the original analysis. Using
the previously described iterative approach, we compute that a sample size of
\mbox{$n = \Sexpr{nex[1]}$} is required under a point design prior at
$\mu_{d} = \Sexpr{dpm}$ to achieve a target power of \Sexpr{round(power*100)}\%.
This differs only slightly from the simulation-based $n = 146$ reported by
\citet{Schoenbrodt2017}, possibly due to Monte Carlo error. The root-finding
approach is, however, substantially faster than the Monte Carlo approach. That
is, computing the sample size for the previous example with root-finding as
implemented in our package (function \texttt{powertbf01}, TODO improve
implementation), takes about 100 milliseconds, while the simulation based
...
density at the null value $\theta_{0} = 0$, encoding the assumption that the
null is the least likely parameter value under the alternative. Normal moment
priors have two modes at $\pm \tau \sqrt{2}$, which gives a convenient way to
elicit a prior, as $\tau$ may, for example, be set to so that the mode equals
two parameter values deemed most plausible under the alternative.
\begin{figure}[!htb]
<< fig.height = 3.5 >>=
...
factor below a threshold $k$ can be expressed in closed-form
\begin{align}
  \label{eq:pnlBF}
  \Pr\left(\mathrm{BF}_{01} \leq k \mid n, \mu_{d}, \tau_{d}\right) =
...
hypothesis accumulates faster with normal moment priors. % The design
...
underlying data), which is a common assumption in the methodology of power and
sample size calculations. We have synthesized and extended previous theoretical
results on power functions for Bayes factors and implemented them in an R
package \texttt{bfpwr}. We also derived novel sample size formulas that
are-easy-to-use, help fostering intuition, and enable asymptotic analysis of
power and sample size. Compared to commonly used simulation-based methods, our
methods are less general. However, in the setting where they are applicable --
which includes many common scenarios, such as testing mean differences -- they
are faster, deterministic, and require no simulation parameters to be specified.
Therefore, we believe that the availability of such methods addresses an
important practical need and can help researchers design efficient studies with
minimal effort.
...
The Bayes factor~\eqref{eq:BF01} with point analysis prior ($\tau^2 = 0$) can be
rewritten as
\begin{align}
...
The Bayes factor~\eqref{eq:BF01} with normal analysis prior ($\tau^2 > 0$) can
be rewritten as
\begin{align}
...
\mbox{$\hat{\theta} \mid n, \mu_{d}, \tau^{2}_{d} \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d} + \sigma^2_{\scriptscriptstyle \hat{\theta}}/n)$}
