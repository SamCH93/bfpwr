...
\date{
  ...
  \anonymize{{\color{blue} Preprint version November 13, 2024}}
}
...
as small sample sizes or when the assumed variance $\sigma^2$ is strongly
...
called `product logarithm'. It has many fundamental applications and has also
...
\Sexpr{round(p, 3)}$ for the null hypothesis of no difference).
...
$f(x)_{[a,b]} = \{f(x) 1_{[a,b]}(x)\}/\{F(b) - F(a)\}$. For example, for a
...
incorporate prior knowledge and uncertainty. %%\citep{Pawel2023e}.
