...
\begin{table}
  ...
  \begin{tabular}{l c c }
    ...
    \midrule
    Point prior &\cellcolor{green!10} Equation~\eqref{eq:nLR2} & \cellcolor{red!10} Unavailable \\
    Normal prior & \cellcolor{green!10} Equation~\eqref{eq:nLR} & \cellcolor{yellow!10} Equation~\eqref{eq:nBF}, for local normal priors \\
    \bottomrule
  \end{tabular}
\end{table}
\endgroup
...
For the Bayes factor with point alternative ($\tau^2 = 0$), the cumulative
...
@
\caption{Examples of Bayes factor power curves. Both Bayes factors contrast
  $H_{0} \colon \theta = \Sexpr{null}$ against
...
@
\caption{Examples of Bayes factor sample size calculations. Both Bayes factors
  contrast $H_{0} \colon \theta = \Sexpr{null}$ against
  $H_{1} \colon \theta \neq \Sexpr{null}$ for a standardized mean difference
  parameter $\theta$ with unit variance
  $\sigma^{2}_{\scriptscriptstyle \hat{\theta}} = 2$ with either a point
  analysis prior ($\tau = \Sexpr{psd1}$, left plot) or a local normal analysis
  prior ($\mu = \Sexpr{null}, \tau = \Sexpr{psd2}$, right plot).}
\label{fig:nexamples}
\end{figure}
...
alternative, and as such, larger prior variances lead to a reduction of sample
...
the point alternative from Table~\ref{tab:smdn} represents a mean difference of
one standard deviation away from the null. The former is a more pessimistic
