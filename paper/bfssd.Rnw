...
% ======================================================================
\begin{abstract}
  \noindent Determining an appropriate sample size is a crucial element of study
  design, and the method used for doing so should be consistent with the planned
  analysis. If the planned analysis involves Bayes factor hypothesis testing,
  the sample size is usually desired to ensure a sufficiently high probability
  of obtaining a Bayes factor that indicates compelling evidence for a
  hypothesis, given that the hypothesis is true. In practice, Bayes factor
  sample size determination is usually performed using computationally intensive
  Monte Carlo simulation. Here we summarize an alternative approach that enables
  sample size determination without simulation. We show how, under approximate
  normality assumptions, sample sizes can be determined numerically, and provide
  the R package \texttt{bfpwr} for doing so. Additionally, we identify
  conditions under which sample sizes can even be determined in closed-form,
  resulting in novel, easy-to-use formulas that also help fostering intuition
  and enable asymptotic analysis. Furthermore, we show how in our framework,
  power and sample size can be computed without simulation for more advanced
  analysis priors, such as Jeffreys-Zellner-Siow priors or nonlocal normal
  moment priors. Case studies from medicine and psychology illustrate how
  researchers can use our methods for planning informative yet cost-efficient
  studies.
  \\
...
setting the power function~\eqref{eq:prLR} equal to a target power, we obtain a
...
nothing that in practice, the sample size formula~\eqref{eq:nLR} will usually
produce non-integer values and hence needs to be rounded to the next larger
integer in order to be an evaluable sample size (e.g., number of participants or
animals).
...
prior is a point prior ($\tau^{2}_{d} = 0$) at $\mu_{d}$, not necessarily the
same as the alternative $\mu$. This leads to~\eqref{eq:nLR} reducing to
\begin{align}
  \label{eq:nLR2}
  n = \frac{\sigma^2_{\scriptscriptstyle \hat{\theta}}
  \left\{z_{1 - \beta} + \sqrt{z_{1 - \beta}^2 - \log k^2 (\Delta_{\mu_{d}}/\Delta_{\mu})}\right\}^2}{\Delta_{\mu_{d}}^2}.
\end{align}
...
$\surd \{z^2_{1-\beta} - \log k^2 (\Delta_{\mu_{d}}/\Delta_{\mu})\}$, reflecting
...
in the design of frequentist hypothesis tests \citep[see e.g.,][for a review of
such Bayesian/frequentist hybrid design approaches]{Grieve2022}.
...
size~\eqref{eq:nLR3} based on an assumed effect size of one
($\Delta_{\mu} = 1$). We see, for instance, that
$n = \Sexpr{nbf01(k = 1/10, power = 0.8, sd = sqrt(2), pm = 1, psd = 0, integer = TRUE)}$
samples per group are required to achieve $1 - \beta = 80\%$ power for a Bayes
factor threshold $k = 1/10$. If the assumed effect size was smaller, the
required sample size would become larger. For example, for a half as large
effect size, the sample sizes from Table~\ref{tab:smdn} quadruple, e.g.,
requiring
...
can be installed from GitHub (\url{https://github.com/SamCH93/bfpwr}), we plan
to submit the package to CRAN in the near future. The workhorse function of our
R package is \texttt{powerbf01}. It is inspired by the \texttt{power.t.test}
