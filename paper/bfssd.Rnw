...
\begin{abstract}
  ...
  asymptotic analysis. We also show how power and sample size can be computed
  without simulation for more advanced analysis priors, such as
  Jeffreys-Zellner-Siow priors or nonlocal normal moment priors. Case studies
  from medicine and psychology illustrate how researchers can use our methods
  for planning informative but cost-efficient studies.
  \\
...
analysis priors, such as informed $t$ priors \citep{Gronau2021} and nonlocal
normal moment priors \citep{Johnson2010}. The paper ends with a closing
...
effective future observations, each with unit variance
...
sample size formula~\eqref{eq:freqsamplesize} assumes continuous outcome data
and a mean difference parameter. The formula could be generalized to other
settings by replacing $2\sigma^{2}$ in the numerator with other unit variances
from Table~\ref{tab:outcomes}, which would then change the interpretation of $n$
and $\hat{\theta}$. It is important to note that these are only approximations
and they may be inadequate in certain situations, such as small sample sizes or
rare events \citep{Spiegelhalter2004}.
...
normal prior where the variance becomes infinitesimally small.
...
when the design prior is also point prior ($\tau^{2}_{d} = 0$),
$Z_{\mathrm{lim}}$~diverges and the limiting power~\eqref{eq:powerlimLR}
...
design prior there is more uncertainty, and hence the power is closer to $50\%$.
...
50\%.
% as there is always more uncertainty with the normal design prior.
...
minus the power curve along with a Bayes factor threshold $k > 1$ and fixing the
design prior to the null hypothesis ($\mu_{d} = \theta_{0}$, $\tau_{d} = 0$), to
determine sample sizes that guarantee a desired probability of convincing
evidence \emph{for the null}. This approach is also implemented in our R
package.
...
$1 - \beta$ lower than the limiting power~\eqref{eq:powerlimLR}, we obtain a
...
\caption{Sample size per group $n$ to obtain a Bayes factor
  ...
  prior are both points priors one standard deviation away from the null value
...
\citep{Pawel2022b, Wagenmakers2022, Pawel2023}.
...
\caption{Power and sample size calculation for example adapted from
  ...
  The power is computed under either a point prior at a medium effect size
  (green), a normal prior that incorporates additional parameter uncertainty
  (blue), or under the null hypothesis (orange). % The design prior is either set
  % to a point prior at $\theta = \Sexpr{dpm}$ or a normal prior at
  % $\mu_{d} = \Sexpr{dpm}$ with standard deviation $\tau_{d} = \Sexpr{dpsd2}$.
  For comparison, \Sexpr{nMC} Monte Carlo simulations are performed with the
  \texttt{BFDA} package \citep{Schoenbrodt2019} to obtain a simulation-based
  power curve approximation (dashed line with one Monte Carlo standard error
  band).}
...
the orange curve in the bottom plot, we see that these same sample sizes only
have modest power of around $20\%$ and $50\%$, respectively, for obtaining
evidence in favor of the true null hypothesis (at a threshold of $k = 6$). To
obtain a power of $80\%$, say, around $n = 800$ samples per group would be
required. This illustrates the well-known fact that evidence accumulates slower
under point null hypotheses compared to the alternative when the Bayes factor
involves testing a point null against a composite alternative with normal
analysis prior \citep{Johnson2010}. Finally, looking at the orange curve in the
top plot and the green/blue curves in the bottom plot, we see that the
probability of misleading evidence in favor of the incorrect hypothesis appears
to be adequately controlled, as these curves are virtually zero over the entire
range of sample sizes.

\section{Extensions}
\label{sec:extensions}
The types of Bayes factor that we considered so far are limited to
asymptotically normally distributed parameter estimates, and normal or point
priors under the alternative. Researchers may also want to use different data
models or prior distributions. In the following we outline two extensions.
...
\begin{enumerate}
  \item For a given sample size $n$ and analysis prior
        ($\mu, \tau, a, b, \kappa$), use numerical root-finding to determine the
        `success region' $S$ in terms of $t$-statistics where the Bayes factor
        is below the threshold $k$, i.e., $S = \{t : \mathrm{BF}_{01} \leq k\}$.
        For a two-sided test, $S$ is typically a region specified by two
        critical values
...
error. The root-finding approach is, however, substantially faster than the
Monte Carlo approach. That is, computing the sample size for the previous
example with root-finding as implemented in our package (function
\texttt{powertbf01}, TODO improve implementation), takes about 100 milliseconds,
while the simulation based approach takes about 10 minutes.
...
Appendix~\ref{app:nmprior} for details. Also normal moment prior Bayes factors
and design based on them is implemented in our R package (functions
\texttt{nmbf01} and \texttt{powernmbf01}, TODO).
...
Figure~\ref{fig:nonlocalexample} again shows power and sample size calculations
...
example, under the point design prior $n = \Sexpr{nH1[1]}$ observations per
group are required while only $n = \Sexpr{n}$ were required with the normal
analysis prior. At the same time, the sample size required to find evidence
\emph{in favor} of the null hypothesis (bottom plot) is drastically reduced
compared to the normal analysis prior (bottom plot in
Figure~\ref{fig:comparisonBFDA}). This is expected since the evidence for a true
null hypothesis accumulates faster with normal moment priors. % The design
% perspective allows data analysts to make an informed choice which of the two is
% more suitable for their aims and resources.
...
methods rely on approximate normality of parameter estimates (but not of the
data), which is a common assumption in the methodology of power and sample size
calculations. We have synthesized and extended previous theoretical results on
power functions for Bayes factors and implemented them in an R package
\texttt{bfpwr}. We also derived novel sample size formulae that are-easy-to-use,
help fostering intuition, and enable asymptotic analysis of power and sample
size. Compared to commonly used simulation-based methods, our methods are less
general. However, in the setting where they are applicable -- which includes
many common scenarios, such as testing mean differences -- they are faster,
deterministic, and require no simulation parameters to be specified. Therefore,
we believe that the availability of such methods addresses an important
practical need and can help researchers design efficient studies with minimal
effort.
...
exists also for $t$-test Bayes factors (\texttt{powertbf01}) and normal moment
...
\citep{Corless1996} . From this expression it is then straightforward to arrive
at the power function~\eqref{eq:pnlBF} when assuming a
$\hat{\theta} \mid n, \mu_{d}, \tau^{2}_{d} \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d} + \sigma^2_{\scriptscriptstyle \hat{\theta}}/n)$
distribution induced by the normal design prior.
