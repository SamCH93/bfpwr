...
\author{
  \textbf{Samuel Pawel} \orcidlink{0000-0003-2779-320X} \and
  \textbf{Leonhard Held} \orcidlink{0000-0002-8686-5325}
}
\date{Epidemiology, Biostatistics and Prevention Institute (EBPI) \\
  Center for Reproducible Science (CRS) \\
  University of Zurich \\
  E-mail: \href{mailto:\mail}{\mail} \\[2ex]
  {\color{blue} Preprint version June 7, 2024}}
...
% ======================================================================
\begin{abstract}
  \noindent An important aspect of study design is the determination of an
  appropriate sample size. The choice of sample size should be consistent with
  the planned analysis. If the planned analysis involves Bayes factor hypothesis
  testing, the sample size is usually desired to ensure a sufficiently high
  probability of obtaining compelling evidence for a hypothesis, given that the
  hypothesis is true. In practice, sample size determination based on Bayes
  factor analyses are typically performed using computationally intensive Monte
  Carlo simulation methods. Here we summarize how, under approximate normality
  assumptions, sample sizes can be determined numerically without simulation,
  and provide the R package \texttt{bfpwr} for doing so. We also identify
  conditions under which sample sizes can be determined in closed-form, leading
  to novel formulas that are easy-to-use, help fostering intuition, and enable
  asymptotic analysis. We also present how for more advanced analysis priors,
  such as Jeffreys-Zellner-Siow priors or non-local normal moment priors, power
  and sample size can be computed without simulation. Case studies from medicine
  and psychology illustrate how researchers can use our methods for planning
  informative but cost-efficient studies.
  \\
  \textit{Keywords:} Bayesian hypothesis testing, evidence, likelihood ratio,
  numerical methods, study design
\end{abstract}
...
A widely used formula for the sample size per group for data with continuous
outcome based on a frequentist hypothesis test of a mean difference for two
equally sized groups is given by
\begin{align}
  \label{eq:freqsamplesize}
  n = \frac{2 \sigma^{2} (z_{1 - \alpha/2} + z_{1 - \beta})^2}{(\mu - \theta_0)^2},
\end{align}
where $z_{q}$ denotes the $q\times 100\%$ quantile of the standard normal
distribution, $\alpha$ is the level of the test, $1 - \beta$ is the desired
power, $\mu$ is the assumed mean difference under the alternative, $\theta_0$ is
the mean difference under the null hypothesis, and $\sigma^2$ is the variance of
one observation \citep[p.~34]{Matthews2006}. There exist various refinements
...
calculations and is often accurate enough for practical purposes. It also helps
fostering intuition and is therefore useful, for example, in teaching of
statistics or the study of asymptotics.

% introduce the Bayes factor and explain why one may want to use it
...
frequentist tests in several ways. For example, they allow us to quantify
evidence in favor of a null hypothesis or they can incorporate external
information via a prior distribution. For an overview of Bayes factors see e.g.,
\citet{Kass1995, Held2018}.
...
knowledge, there are no simple formulae such as~\eqref{eq:freqsamplesize} for
...
parameter under the alternative used in the analysis) and the design prior (the
prior distribution for the parameter used in the design). Specifically, for
Bayes factors with point analysis priors (i.e., likelihood ratios) and point or
normal design priors, and for Bayes factors with local normal analysis and
design priors (i.e., normal priors centered around the null value) closed-form
sample sizes are available, see Table~\ref{tab:summary} for an overview of our
results. We also summarize sample size determination for Bayes factors analysis
based on numerical root-finding \citep{Weiss1997} that, while not being
available in closed-form, is much faster and deterministic compared to
simulation-based approaches. To facilitate reuse of our results, all methods are
made available through the R package \texttt{bfpwr}.

\begingroup
\renewcommand{\arraystretch}{1.3} % Default value: 1
\begin{table}[!htb]
  \centering
  \caption{Availability of closed-form sample size formulae for Bayes factor
    hypothesis test of $H_{0} \colon \theta = \theta_{0}$ against
    $H_{1} \colon \theta \neq \theta_{0}$ with either a point prior or a normal
    prior assigned to $\theta$ under $H_{1}$, and data in the form of a normally
    distributed parameter estimate
    $\hat{\theta} \mid \theta \sim \mathrm{N}(\theta, \sigma_{\scriptscriptstyle \hat{\theta}}^{2}/n)$. In all cases, the power can be computed in closed-form.
    % with standard error $\sigma_{\scriptscriptstyle \hat{\theta}}/\surd n$.
  }
  \label{tab:summary}
  \begin{tabular}{l c c }
    \toprule
    ...
    \cmidrule{2-3}
    Design prior & Point prior & Normal prior \\
    % & $\theta = \mu$ & $\theta \sim \mathrm{N}(\mu, \tau^{2})$ \\
    \midrule
    Point prior % $\theta = \mu_{d}$
 &\cellcolor{green!10}  \cmark ~ Equation~\eqref{eq:nLR2} & \cellcolor{red!10} \xmark ~ Unavailable \\
    Normal prior % $\theta \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d})$
    & \cellcolor{green!10} \cmark ~ Equation~\eqref{eq:nLR} & \cellcolor{yellow!10} \cmark ~ Equation~\eqref{eq:nBF}, but only for local normal priors \\
    \bottomrule
  \end{tabular}
\end{table}
\endgroup
...
by deriving its distribution when new data are generated under different design
priors (Section~\ref{sec:distBF}). Combining these results, we derive several
formulae for the sample size under different constellations of design and
analysis priors (Section~\ref{sec:ssd}). Examples from medicine and the social
sciences then illustrates how our formulae can be implemented by researchers in
practice (Section~\ref{sec:application}). Section~\ref{sec:extensions}
illustrates possible extensions of our framework to other popular types of
analysis priors, such as informed $t$ \citep{Gronau2021} and non-local normal
moment priors \citep{Johnson2010}. The paper ends with a closing discussion of
our results and final remarks on limitations and extensions
(Section~\ref{sec:discussion}). Our R package \texttt{bfpwr} that implements the
developed methodology, is illustrated in Appendix~\ref{app:pkg}.


\section{Assumed Bayes factor analysis}
\label{sec:BF}
To derive a formula for sample size calculation, we must first clarify how the
future data will be analyzed. Denote by $\hat{\theta}$ the estimate of an
unknown parameter $\theta$ that will result from the statistical analysis of $n$
effective future samples, each with unit variance
$\sigma^2_{\scriptscriptstyle \hat{\theta}}$. Assume that the estimate is
approximately normally distributed
$\hat{\theta} \mid \theta \sim \mathrm{N}(\theta, \sigma^2_{\scriptscriptstyle \hat{\theta}}/n)$.
...
$\sigma^2_{\scriptscriptstyle \hat{\theta}}$. For example, the frequentist
sample size~\eqref{eq:freqsamplesize} assumes continuous outcome data and a mean
difference parameter. The formula could be generalized to other settings by
replacing $2\sigma^{2}$ in the numerator with other unit variances from
Table~\ref{tab:outcomes}. It is important to note that these are only
approximations and they may be inadequate in certain situations, such as small
sample sizes or rare events \citep{Spiegelhalter2004}.
...
\begin{table}[!htb]
  ...
  \caption{Different types of parameter estimates $\hat{\theta}$ with
    ...
    and parameter estimates based on two groups assume equal number of samples
    per group.}
  \label{tab:outcomes}
  ...
  \begin{tabular}{l l l c}
    ...
    \midrule
    Continuous & Mean & Number of samples & $\sigma^{2}$ \\
    ...
    Binary & Arcsine square root difference & Number of samples per group & 1/2 \\
...
the alternative $H_{1}$, while a Bayes factor greater than one
...
and the standard error is obtained numerically (e.g., with Fisher scoring). In
this case, $\sigma_{\scriptscriptstyle \hat{\theta}}/\sqrt{n}$ in~\eqref{eq:BF01}
can be replaced by the observed standard error. However, as we will show now,
assuming such a particular dependence on the sample size $n$ allows us to
perform closed-form power and sample size calculations under certain additional
assumptions.
...
Assume a so-called `design prior' for the parameter $\theta$ that is used for
the design of the study \citep{OHagan2001, OHagan2005}. This prior should
represent the state of knowledge and uncertainty about $\theta$ at the design
stage and does not necessarily have to correspond to the `analysis prior'
...
for the future parameter estimate $\hat{\theta}$. Under this normal sampling
distribution of the future estimate, the distribution of the Bayes factor with
normal analysis prior~\eqref{eq:BF01} can be derived in closed-form
\citep{Weiss1997, DeSantis2004}. We now rederive and extend this result in our
setting and notation. The two cases of the Bayes factor with $\tau^2 = 0$ (point
analysis prior under the alternative) and $\tau^2 > 0$ (normal analysis prior
under the alternative) need to be distinguished, as the resulting distributions
take a different form, and only the latter has been considered previously in the
Bayes factor literature.

For the Bayes factor with point analysis prior ($\tau^2 = 0$), the cumulative
distribution or `power function' is
\begin{align}
\label{eq:prLR}
  \Pr(\text{BF}_{01} \leq k \mid n, \mu_{d} , \tau_{d}, \tau^{2} = 0)
  = \begin{cases}
...
\begin{align}
  \label{eq:powerlimLR}
  \lim_{n \to \infty} \Pr(\mathrm{BF}_{01} \leq k \mid n, \mu_{d} , \tau_{d}, \tau^{2} = 0)
    = \begin{cases}
      1 - \Phi(Z_{\mathrm{lim}}) & \text{if}~ \mu - \theta_0 > 0 \\
      \Phi(Z_{\mathrm{lim}}) & \text{if}~ \mu - \theta_0 < 0
    \end{cases}
\end{align}
with $Z_{\mathrm{lim}} = (\theta_{0} + \mu - 2 \mu_{d})/(2 \tau_{d})$. Clearly,
when the design prior is a also point prior ($\tau^{2}_{d} = 0$),
$Z_{\mathrm{lim}}$~diverges and the limitting power~\eqref{eq:powerlimLR}
approaches one or zero, depending on whether the location of the design prior
$\mu_{d}$ is closer to the alternative $\mu$ or to the null $\theta_{0}$. In
case it is just in between the two, the limiting power approaches a half. On the
other hand, for a normal design prior ($\tau^{2}_{d} > 0$), the limiting power
...
parallels similar results on bounds for Bayesian/frequentist hybrid power of
significance tests \citep{Spiegelhalter2004, Micheloud2020, Grieve2022}.
...
\caption{Examples of Bayes factor power curves computed with
  ...
  analysis prior ($\tau^{2} = \Sexpr{psd1}$, left plot) or a normal analysis
...
design prior there is always more uncertainty, and hence the power is always
closer to $50\%$.

For the Bayes factor with normal analysis prior ($\tau^2 > 0$), the
...
\begin{align}
  \lim_{n\to \infty} \Pr(\text{BF}_{01} \leq k \mid n, \mu_{d} , \tau_{d}, \tau^{2} > 0) = 1
  \label{eq:plimBF}
\end{align}
regardless of whether the design prior is a point prior ($\tau^{2}_{d} = 0$) or
a normal prior ($\tau^{2}_{d} > 0$), provided that the design prior is not equal
to the point null hypothesis itself. This is expected because Bayes factors
testing point nulls against composite alternatives are `consistent' in the sense
that as the sample size increases, the probability of the Bayes factor favoring
the hypothesis under which the data were generated tends to one
...
analysis prior, and computer under either point (red curve) or normal design
...
normal than under the point design prior because there is more parameter
uncertainty. Finally, as with the point analysis prior Bayes factor, the curves
cross at 50\% as there is always more uncertainty with the normal design prior.
...
specified design prior \citep{Weiss1997}. Finally, one may also look at one
minus the power curve with $k> 1$ to compute the power to find evidence for the
null, along with fixing the design prior to the null hypothesis, to determine
sample sizes that guarantee a desired probability of convincing evidence for the
null. This approach is also implemented in our R package.
...
Bayes factors with point analysis priors ($\tau^2 = 0$) and normal analysis priors
($\tau^2 > 0$), we start again with the former.

\subsection{Bayes factor with point analysis prior}
% % Suppose that we are interested in the Bayes factor with point analysis prior
% % ($\tau^{2}$) ,
% Assume a normal design prior $\theta \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d})$.
% This prior may be set equal to the point analysis prior used in the analysis
% ($\mu_{d} = \mu$ and $\tau^{2}_{d} = 0$) or it may incorporate additional prior
% information and uncertainty. This leads to a predictive distribution with mean
% $m = \mu_{d}$ and variance $v = \tau^{2}_{d} + \sigma^{2}/n$.
Assuming that the alternative $\mu$ is larger than the null $\theta_{0}$ and
setting the power function~\eqref{eq:prLR} equal to a target power of
$1 - \beta$ lower than the limitting power~\eqref{eq:powerlimLR}, we obtain a
quadratic equation in the sample size $n$
\begin{align*}
...
\end{align}
Of note, the second solution of the quadratic equation gives the sample size
that leads to a power of $\beta$ instead of $1 - \beta$. The sample
size~\eqref{eq:nLR} also typically needs to be rounded to the next larger
integer in order to be an evaluable sample size in practice.

We will now investigate the sample size~\eqref{eq:nLR} for two special cases.
First, suppose that the design prior is a point prior ($\tau^{2}_{d} = 0$) at
$\mu_{d}$, not necessarily the same as the alternative $\mu$. This leads to
\begin{align}
  \label{eq:nLR2}
  n = \frac{\left[z_{1 - \beta} + \sqrt{z_{1 - \beta}^2 - \log k^2 \{(\theta_0 + \mu - 2\mu_{d})/(\theta_0 - \mu)\}}\right]^2}{(\mu + \theta_0 - 2\mu_{d})^2 / \sigma^2_{\scriptscriptstyle \hat{\theta}}}.
\end{align}
Assuming now that the tested parameter is a mean difference with unit variance
$\sigma^2_{\scriptscriptstyle \hat{\theta}} = 2\sigma^{2}$ (see
Table~\ref{tab:outcomes}, second row), we can see that the
formula~\eqref{eq:nLR2} represents a modification of the frequentist
formula~\eqref{eq:freqsamplesize}: The `standardized effect size'
$(\mu - \theta_0)/ \sigma$ in the denominator is replaced by a standardized
effect size that takes into account the mean of the design prior
$(\mu + \theta_0 - 2\mu_{d})/\sigma$, but reduces to the standardized effect
size when the assumed parameter in the design equals the assumed parameter in
the analysis (when $\mu_{d} = \mu$). Moreover, the quantile $z_{1-\alpha/2}$ is
replaced by
$\surd [z^2_{1-\beta} - \log k^2 \{(\theta_0 + \mu - 2m)/(\theta_0 - \mu)\}]$,
reflecting the fact that we are interested in a Bayes factor hypothesis test and
not a frequentist significance test.
...
\begin{align}
\label{eq:nLR3}
    n = \frac{\left\{z_{1 - \beta} + \sqrt{z_{1 - \beta}^2 - \log k^2}\right\}^2}{(\mu - \theta_0)^2 / \sigma^2_{\scriptscriptstyle \hat{\theta}}}.
\end{align}
The same formula~\eqref{eq:nLR3} was also found by \citet{Strug2007} for
`evidential' sample size calculations, but is unfortunately not well known.
Unsurprisingly the two formulae coincide as Bayes factors and likelihood ratios
-- the measure of evidence used in `evidential statistics' \citep[see
e.g.,][]{Edwards1971, Royall1997, Blume2002, Strug2018} -- coincide when the
Bayes factor involves only point hypotheses. Our more general
formulae~\eqref{eq:nLR} and~\eqref{eq:nLR2} thus also generalize evidential
sample size calculations to incorporate different parameters and/or parameter
uncertainty, similar to how Bayesian design priors can be used for incorporating
parameter uncertainty in a planned frequentist analysis \citep[see e.g.,][for an
overview of Bayesian/frequentist hybrid design approaches]{Grieve2022}.


\begin{table}[ht]
\centering
\caption{Sample size per group $n$ to obtain a Bayes factor $\mathrm{BF}_{01}$
  $k$ with at least a power of $1 - \beta$. The parameter of interest is a
  standardized mean difference and the analysis and design prior are both points
  priors one standard deviation away from the null value
  ($\mu - \theta_{0} = 1$) so that equation~\eqref{eq:nLR3} can be used.}
\label{tab:smdn}
...
as the number of samples per group (see the third row in
Table~\ref{tab:outcomes}). Table~\ref{tab:smdn} shows the sample
size~\eqref{eq:nLR3} based on an assumed standardized mean difference of one
($\mu - \theta_{0} = 1$). We see, for instance, that $n = 20$ samples per group
are required to achieve $1 - \beta = 80\%$ power for a Bayes factor threshold
$k = 1/10$. If the assumed standardized mean difference were smaller, the
required sample size would become larger. For example, for a half as large
standardized mean difference, the sample sizes from Table~\ref{tab:smdn}
quadruple, e.g., requiring $n = 80$ samples per group to have $1 - \beta = 80\%$
power for a Bayes factor threshold of $k = 1/10$.
...
illustrates the sample size formula~\eqref{eq:nLR} that incorporates additional
...
In general, there is no closed-form solution. However, as we will show now,
solutions can, under certain conditions, be expressed in terms of the Lambert W
function \citep{Corless1996}. The Lambert W function is the function
$\mathrm{W}(\cdot)$ that satisfies $\mathrm{W}(x)\exp\{\mathrm{W}(x)\}=x$, and
it is therefore sometimes also called `product logarithm'. It has many
fundamental applications, such as the solution of the Schrödinger equation in
quantum-mechanics, and has also previously appeared in the context of Bayes
factor hypothesis testing \citep{Pawel2022b, Wagenmakers2022, Pawel2023}.
...
\begin{align}
  \label{eq:prBFcenter}
    \Pr\{\text{BF}_{01} \leq k \mid n, \tau^{2}_{d}, \mu_{d} = \mu = \theta_{0}\}
    = 2\Phi(-\sqrt{X}).
\end{align}
Further, assume that the variance of the design prior corresponds to the
variance of the analysis prior ($\tau^2_{d} = \tau^{2}$). We then have that
\begin{align*}
...
($\tau^{2} = \sigma^{2}_{\scriptscriptstyle \hat{\theta}}$). As in classical
sample size calculations, smaller unit variances
$\sigma^{2}_{\scriptscriptstyle \hat{\theta}}$ reduce the sample size. The prior
variance $\tau^{2}$ determines how large parameters are expected under the
alternative hypothesis, and as such, larger prior variances lead to a reduction
of sample size similar to the effect size in classical sample size
determination. Finally, the formula~\eqref{eq:nBF} allows us to study the
potential existence of sample size that can achieve the target power: Since the
argument of the Lambert W function has to be at least $-1/e$ for it to be
defined, we can infer that only combinations of Bayes factor thresholds $k$ and
power values $1 - \beta$ that satisfy $k^{2} z^{2}_{(1 - \beta)/2} \leq 1/e$ can
actually be achievable with a finite sample size. For example, it is impossible
to find a sample size that guarantees a power of $1 - \beta = 50\%$ for a
threshold of $k = 1$ since then
$z_{0.25} = \Sexpr{round(qnorm(0.25), 2)} < 1/\sqrt{e} = \Sexpr{round(1/sqrt(exp(1)), 3)}$.


\begin{table}[ht]
\centering
\caption{Required unit information sample size $n_{k,\beta}$ to obtain a Bayes
  factor smaller than $k$ with at least a power of $1-\beta$ with a unit
  information analysis and design prior.}
\label{tab:nnormal}
...
prior represents a parameter distribution that is centered around the null value
while the point analysis prior from Table~\ref{tab:smdn} represents a mean
difference of one standard deviation away from the null. The former is a more
pessimistic assumption than the latter. To incorporate more optimistic beliefs
into the calculations we may increase the standard deviation $\tau$ of the
distribution as this encodes the assumption of potentially larger parameters.
For example, doubling $\tau$ leads to a four-fold decrease of the sample
size~\eqref{eq:nBF}. This is also illustrated in the right plot of
...
transcendental nature of the power equation, even in terms of the Lambert W
function. Fortunately, the sample size can still be easily calculated
numerically with our R package which is quicker and more reliable than computing
it with a simulation approach.


% However, we may also want to use a different design prior than the one under
% the alternative. One possibility is to ignore the term in~\eqref{eq:prBF} in
% the opposite direction of the prior under the alternative as this term may be
% close to zero. A similar assumption is made when deriving the classical sample
% size formula.
...
relief between the treatment and placebo groups $\theta$. The null hypothesis
was defined as no difference in efficacy of the treatment
($H_{0} \colon \theta = 0$), whereas the alternative hypothesis was defined as a clinically
relevant difference of one day ($H_{1} \colon \theta = 1$). For sample size
calculations, the investigators used a standard deviation of $\sigma = 2.75$
...
\caption{Power and sample size calculations for MIST trial \citep{MIST1998}. The
  ...
  Curves are shown for data distributions induced by a point design prior at the
  alternative $\mu_{d} = \mu = \Sexpr{pm}$, a normal design prior centered at
  the alternative with standard deviation $\tau_{d} = \Sexpr{dpsd2}$, and under
  the null hypothesis.}
\label{fig:MIST}
\end{figure}
...
colors indicate under which data distribution the power was computed.

Focusing on the top plot, we can see from the green curve that at least
$n = \Sexpr{nanalyt}$ samples per group are required to ensure a target power of
$1 - \beta = \Sexpr{round(power*100,2)}\%$ assuming that the same point prior is
used in the design as for the analysis (i.e., a point prior at
...
data are generated from the design priors seems to be reasonably well controlled
(below 5\%) across the whole range of sample sizes considered.
...
achieved. This approach can be used in quite general settings, but it is also
...
\caption{Power and sample size calculation for example adapted from
  ...
  $H_{1} \colon \theta \neq \Sexpr{null}$ with
  $\theta \mid H_{1} \sim \mathrm{N}(\Sexpr{pm}, 1/\Sexpr{1/psd^2})$ prior
  assigned to the standardized mean difference $\theta$ under the alternative.
  The design prior is either set to a point prior at $\theta = \Sexpr{dpm}$ or a
  normal prior at $\mu_{d} = \Sexpr{dpm}$ with standard deviation
  $\tau_{d} = \Sexpr{dpsd2}$. For comparison, \Sexpr{nMC} Monte Carlo
  simulations are performed with the \texttt{BFDA} package
  \citep{Schoenbrodt2019} to obtain a simulation-based power curve approximation
  (shown with one Monte Carlo standard error bands).}
...
one-sided Cauchy prior with scale $1/\sqrt{\Sexpr{1/psd^2}}$ that was assumed by
\citet{Schoenbrodt2017}. As they did, we also investigate two design priors:
either a point prior at $\theta = \Sexpr{dpm}$ , which is a convention for a
`medium` standardized mean difference \citep{Cohen1992}, or a normal prior at
$\mu_{d} = \Sexpr{dpm}$ with standard deviation $\tau_{d} = \Sexpr{dpsd2}$ to
incorporate parameter uncertainty. Figure~\ref{fig:comparisonBFDA} shows the
resulting power curves computed from equation~\eqref{eq:prBF}, along with the
\texttt{BFDA} Monte Carlo simulation approximations (with Monte Carlo standard
error bands) for comparison. The closed-form power curves were computed
instantly while each simulated power curve took about 10 minutes to compute when
parallelized across 10 cores on a modern laptop.
...
their calculations, which instead assumed a one-sided Cauchy analysis prior. As
in the previous example, incorporating parameter uncertainty via a normal design
prior increases the required sample size, in this case to $n = \Sexpr{n2}$.
Looking at the orange curve in the bottom plot, we see that these same sample
sizes only have modest power of around $20\%$ and $50\%$, respectively, for
obtaining evidence in favor of the true null hypothesis (at a threshold of
$k = 6$). To obtain a power of $80\%$, say, around $n = 800$ samples per group
would be required. This illustrates the well-known fact that evidence
accumulates slower under point null hypotheses compared to the alternative when
the Bayes factor involves testing a point null against a composite alternative
with normal analysis prior \citep{Johnson2010}. Finally, looking at the orange
curve in the top plot and the green/blue curves in the bottom plot, we see that
the probability of misleading evidence in favor of the incorrect hypothesis
appears to be adequately controlled, as these curves are virtually zero over the
entire range of sample sizes.

\section{Extensions}
\label{sec:extensions}
The type of Bayes factor that we considered so far is limited to asymptotically
normally distributed parameter estimates, and normal or point priors under the
alternative. Researchers may also want to use different data models or prior
distributions. In the following we outline two extensions.

\subsection{Informed Bayesian \textit{t}-test}

\citet{Gronau2021} proposed a Bayes factor for testing a standardized mean
...
\begin{align}
  ...
  \mathrm{NCT}_{\nu}(t \mid \sqrt{n} \theta) \, \mathrm{T}_{\kappa}(\theta \mid \mu, \tau)_{[a,b]}
  \, \mathrm{d}\theta}
  \label{eq:tBF}
\end{align}
with $t$ the $t$-test statistic, $n$ the effective sample size (the actual
number of observations/pairs for one-sample or paired $t$-tests, or
...
`Jeffreys-Zellner-Siow' Bayes factor \citep{Jeffreys1961, Zellner1980}, which is
often used as a `default' Bayes factor in the social sciences
\citep{Rouder2009}. Setting other values for these parameters allows data
analysts to incorporate directionality or prior knowledge about the standardized
mean difference, potentially improving the efficiency of the test
\citep{Stefan2019}. The Bayes factor~\eqref{eq:tBF} is also implemented in our
package in the function \texttt{tbf01}.
...
numerical evaluation of the integral in the denominator. To compute the power
for a given sample size, we may hence use the following two-stage procedure:

\begin{enumerate}
  \item For a given sample size $n$ and analysis prior ($\mu, \tau, a, b$), use
        numerical root-finding to determine the `success region' $S$ in terms of
        $t$-statistics where the Bayes factor is below the threshold $k$, i.e.,
  ...
  \item For a given design prior, compute the probability of obtaining a
        $t$-statistic contained in $S$. For example, based on a normal design
        prior $\theta \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d})$, assuming a
        two-sample test with equally sized groups and that the data variance
        $\sigma^{2}$ is known, we have that
  \begin{align*}
    \Pr\left(\mathrm{BF}_{01} \leq k \mid n, \mu_{d}, \tau_{d} \right)
    = \Phi\left(\frac{(2t_{\mathrm{crit}-})/\sqrt{n} - \mu_{d}}{\sqrt{\tau^{2}_{d} + 2/n}}\right)
    + \Phi\left(\frac{\mu_{d} - (2t_{\mathrm{crit}+})/\sqrt{n}}{\sqrt{\tau^{2}_{d} + 2/n}}\right).
  \end{align*}
\end{enumerate}
...
specify a one-sided scaled Cauchy prior ($\kappa = 1$, $\mu = 0$, $a = 0$,
$b = +\infty$) with scale $\tau = 1/\sqrt{2}$ as in the original analysis. To
achieve a target power of \Sexpr{round(power*100)}\%, a sample size of
$n = \Sexpr{nex[1]}$ is required under the point design prior at
$\mu_{d} = \Sexpr{dpm}$ differing only slightly from the Monte Carlo based
$n = 146$ estimate reported by \citet{Schoenbrodt2017}, possibly due to
numerical or Monte Carlo error . The root-finding approach is, however,
substantially faster than the Monte Carlo approach. That is, computing the
sample size for the previous example with root-finding as implemented in our
package (function \texttt{powertbf01}), takes about 100 milliseconds, while the
simulation based approach takes about 10 minutes.
...
form
$\mathrm{NM}(\theta \mid \theta_{0}, \tau^{2}) = \mathrm{N}(\theta \mid \theta_{0}, \tau) \times (\theta - \theta_{0})^{2}/\tau^{2}$
with location $\theta_{0}$ and spread $\tau$. Figure~\ref{fig:nonlocal} shows
three examples of normal moment priors. We see that the distributions have zero
density at the null value $\theta_{0} = 0$, encoding the assumption that
parameters under the alternative are least likely to be exactly null. Normal
moment priors have two modes at $\pm \tau \sqrt{2}$, which gives a convenient
way to elicit a prior, as $\tau$ may, for example, be set to so that the mode
equals parameter value deemed most plausible under the alternative.
\begin{figure}[!htb]
<< fig.height = 3.5 >>=
...
@
\caption{Illustration of a normal moment prior density with different spread
  parameter $\tau$. }
\label{fig:nonlocal}
\end{figure}
...
\end{align}
see e.g., \citet{Pramanik2024}. Under a normal design prior
$\theta \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d})$, the power to obtain a Bayes
factor below a threshold $k$ can (after some nasty calculations) be expressed in
closed-form
\begin{align}
  \label{eq:pnlBF}
  \Pr\left(\mathrm{BF}_{01} \leq k \mid n, \mu_{d}, \tau_{d}\right) =
...
Appendix~\ref{app:nmprior} for details.
...
We presented methods for performing power and sample size calculations for
settings where future data are analyzed with Bayes factor hypothesis tests.
These methods rely on approximate normality of the data, which is a common
assumption in the methodology of power and sample size calculations. We have
synthesized and extended previous theoretical results on power functions for
Bayes factors and implemented them in an easy-to-use R package \texttt{bfpwr}.
We also derived novel sample size formulae that are-easy-to-use, help fostering
intuition, and enable asymptotic analysis of power and sample size. Compared to
commonly used simulation-based methods, our methods are less general. However,
in the setting where they are applicable -- which includes many common
scenarios, such as testing mean differences -- they are faster, deterministic,
and require no simulation parameters to be specified. Therefore, we believe that
the availability of such methods addresses an important practical need and can
help researchers design efficient studies with minimal effort.
...
extend closed-form sample size calculations to more specific settings, such as,
binary outcomes. Another limitation is the types of Bayes factors that we
considered for the analysis, which is limited to univariate parameters with
normal, normal moment, $t$, or point priors under the alternative. Our work
could be extended to ANOVA or regression settings with multivariate parameters
and/or to other prior distributions. We also did not consider `open-ended'
sequential designs, where data are collected continuously until compelling
evidence for one of the competing hypotheses is found \citep{Wald1947}. The
sequential approach is particularly attractive for Bayes factor inference as
design and analysis prior distributions can be updated based on the accumulating
data. Researchers can then make informed decisions about whether or not it is
worthwhile to continue collecting data or to stop \citep{Stefan2022}. For these
purposes, it would be interesting to consider the Bayes factor indexed by the
sample size as a stochastic process, and study its properties. Finally, in
situations where researchers have only a fixed sample size at their disposal, it
may be interesting to use a `reverse-Bayes' approach \citep{Held2021b}. That is,
one may go backwards and determine the design prior required to achieve a
desired target performance. Researchers can then reason whether or not this
`reverse Bayes design prior' is scientifically plausible, and whether or not
they should undertake the study based on their limited resources.


\section*{Acknowledgments}
We thank František Bartoš for valuable comments on drafts of the manuscript. The
...
Code and data to reproduce our analyses are openly available
at \url{https://github.com/SamCH93/bfpwr}. A snapshot of the repository at the
time of writing is available at \url{https://doi.org/10.5281/zenodo.XXXXXX}. We
used the statistical programming language
\Sexpr{R.Version()[["version.string"]]} for analyses \citep{R} along with the
...
an R session, the development version can be installed from GitHub
(\url{https://github.com/SamCH93/bfpwr}). The workhorse function of our R
package is \texttt{powerbf01}. It is inspired by the \texttt{power.t.test}
...
are continuous and that the parameter of interest is either a mean or a mean
difference. The functions \texttt{pbf01} and \texttt{nbf01} are more general and
can be used for any approximately normally distributed parameter estimate with
approximate variance
$\mathrm{Var}(\hat{\theta}) = \sigma^{2}_{\scriptscriptstyle \hat{\theta}}/n$,
although users have to specify the unit variance
$\sigma^{2}_{\scriptscriptstyle \hat{\theta}}$ themselves. The following code
chunk illustrate how \texttt{powerbf01} can be used.
...
The Bayes factor~\eqref{eq:BF01} with $\tau^2 = 0$ can be rewritten as
\begin{align}
...
$\text{BF}_{01} \leq k < 1$. In this case, $\text{BF}_{01} \leq k$ can be
rewritten as
\begin{align*}
...
$\hat{\theta} \mid n, \mu_{d}, \tau^{2}_{d} \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d} + \sigma^2_{\scriptscriptstyle \hat{\theta}}/n)$,
...
\end{align*}
Thus, when $\tau^{2}_{d} \downarrow 0$ and $\mu_{d} \neq \theta_{0}$, both $M$
and $X$ diverge but the $M$ term diverges faster than the $X$ term. When
$\tau^{2}_{d} > 0$, the $M$ term approaches a constant while the $X$ term
approaches zero. Consequently, in both cases it holds that
\begin{align*}
  \lim_{n\to \infty}  \Pr(\text{BF}_{01} \leq k \mid n, \mu_{d} , \tau_{d}, \tau^{2} > 0)
...
Equating the power function~\eqref{eq:prBFcenter} to $1 - \beta$ and
...
$\mathrm{W}_{-1}(\cdot)$ which satisfies $\mathrm{W}(x) < -1$ for $y \in (-1/e, 0)$
\citep{Corless1996}. This is because this branch always leads to larger samples
sizes than the other and guarantees that unit information sample sizes are
...
there is the principal branch $\mathrm{W}_{0}$ that can satisfy the inequality
\citep{Corless1996} . Isolating the parameter estimate $\hat{\theta}$ on the
left-hand-side further, it is straightforward to arrive at the power
function~\eqref{eq:pnlBF} when assuming a normal design prior as the
distribution of $\hat{\theta}$ is then also normal.
