...
although the latter shows Monte Carlo error. Looking at the green curves in the
...
Of note, after publishing an initial version of this article on the arXiv
preprint server, \citet{Wong2024} have already built on our results and extended
the approach described in this section to use more flexible design prior
distributions for the $t$-test setting, and implemented it in an online
application. We refer to their article for more information.
...
univariate parameters. Our work could be extended to ANOVA or regression
settings with multivariate parameters. We also did not consider `open-ended'
...
desired target power. Researchers can then reason whether or not this `reverse
Bayes design prior' is scientifically plausible, and whether or not they should
undertake the study based on their limited resources. This is similar to
classical power analysis, where one can determine how large the true parameter
needs to be to achieve a given power for a given sample size, and then reason
about whether or not this true parameter is realistic. In both cases, however,
it is important that the focus is on reasoning about the plausibility of the
design prior/parameter, and researchers should be careful not to give the
impression that this choice was made at the outset.
