...
practice (Section~\ref{sec:application}). The paper ends with a closing
discussion of our results and final remarks on limitations and extensions
...
prior \mbox{$\theta \mid H_1 \sim \mathrm{N}(\mu, \tau^2)$} assigned to $\theta$
under $H_1$. The mean of the prior $\mu$ determines the most plausible parameter
value under the alternative while the variance $\tau^{2}$ determines its
uncertainty. Informally, a point alternative at $\mu$ can be obtained by setting
the variance of the prior to zero.
...
$\sigma_{\scriptscriptstyle \hat{\theta}}/\surd n$, e.g, a parameter estimate
...
this case, $\sigma_{\scriptscriptstyle \hat{\theta}}/\surd n$ in~\eqref{eq:BF01}
...
to the null $\theta_{0}$. In case it is just in between the two, the limiting
power approaches a half. On the other hand, for a normal design prior
($\tau^{2}_{d} > 0$), the limiting power is bounded by a value in between (and
not including) zero and one given by~\eqref{eq:powerlimLR}. The intuition behind
these results is that for a normal design prior, there is always parameter
uncertainty, even if the sample size becomes arbitrarily large, while for point
design priors, the parameter uncertainty can be arbitrarily reduced by
increasing the sample size. This parallels similar results on bounds for
Bayesian/frequentist hybrid power of significance tests
...
(red curve) or a normal design prior (blue curve), and the curves show different
qualitative behaviors: We see that increasing the sample size $n$ increases the
power in both cases. However, as known from~\eqref{eq:powerlimLR}, the power
under the normal design prior is bounded by
$1 - \Phi\{(\theta_{0} + \mu - 2\mu_{d})/(2\tau_{d})\} = \Sexpr{round(plim*100,1)}\%$
while the power under the point design prior can be increased to $100\%$.
Finally, the curves cross at 50\%, the intuition being that with the normal
design prior there is always more uncertainty about the future Bayes factor, and
hence the power is always closer to $50\%$.
...
analysis prior, and computer under either point or normal design prior. We see
that in all cases increasing the sample size increases the power. As expected
from~\eqref{eq:plimBF}, the power can increase to $100\%$ in both cases,
although it approaches the limit much slower under the normal (blue curve) than
under the point design prior (red curve) because there is more parameter
...
specified design prior \citep{Weiss1997}. This is also implemented in our R
package.
...
Suppose now that the design and analysis prior are both centered around
...
variance of the analayis prior  ($\tau^2_{d} = \tau^{2}$). We
then have that
\begin{align*}
...
alternative hypothesis, and as such, larger prior variances lead to a reduction of sample
size similar to the effect size in classical sample size determination. Finally,
the formula~\eqref{eq:nBF} allows us to study the potential existence of sample
size that can achieve the target power: Since the argument of the Lambert W
function has to be at least $-1/e$ for it to be defined, we can infer that only
...
calculations encode vastly different assumptions: The normal analysis prior
represents a parameter distribution that is centered around the null value while
the point analysis prior from Table~\ref{tab:smdn} represents a mean difference
of one standard deviation away from the null. The former is a more pessimistic
assumption than the latter. To incorporate more optimistic beliefs into the
calculations we may increase the standard deviation of the distribution $\tau$
as this encodes the assumption of potentially larger parameters. For example,
doubling the standard deviation $\tau$ leads to a four-fold decrease of the
...
analysis priors~\eqref{eq:nLR}. This is because it makes the restrictive assumption
...
calculation. However, we have not been able to derive a sample size formula for
this more general setting due to the transcendental nature of the power
...
Cauchy prior with scale $1/\surd{\Sexpr{1/psd^2}}$ that was assumed by
...
null hypotheses compared to the alternative when the Bayes factor involves
testing a point null against a composite alternative with normal analysis prior
\citep{Johnson2010}. Finally, looking at the orange curve in the top plot and
the green/blue curves in the bottom plot, we see that the probability of
misleading evidence in favor of the incorrect hypothesis appears to be
adequately controlled, as these curves are virtually zero over the entire range
of sample sizes.
...
% \subsection{Non-local moment priors}
...
though users have to specify the unit variance
