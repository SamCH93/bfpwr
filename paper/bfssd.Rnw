...
($\mu_{d} = 0$ and $\tau_{d} = 0$) allows us to compute the probability of
finding compelling evidence for the true null hypothesis, as well as the
probability of finding misleading evidence for the alternative when the null
hypothesis is true (the `type I error rate'). Presenting the latter calculation
can be useful in convincing skeptical parties (e.g., regulators, research
funders, frequentist statisticians) that the chosen design and analysis are
appropriately 'calibrated' \citep{Dawid1982, Rubin1984, Little2006, Grieve2016}.
...
$\sigma = 2.75$ estimated from a previous study. With a target power of
$1 - \beta = 90\%$ and a two-sided level of $\alpha = 0.05$, the frequentist
formula~\eqref{eq:freqsamplesize} produces a sample size
$n = \Sexpr{ceiling(2*2.75^2*(qnorm(0.9) + qnorm(0.975))^2)}$, which the
investigators adjusted further upwards to account for potential dropout and the
non-parametric test used in the analysis. In the end, the trial indicated
evidence for a beneficial effect of Zanamivir compared to placebo
(intention-to-treat analysis difference in median number of days to alleviation
of symptoms of 1.5 with 95\% confidence interval from 0.5 to 2.25).

While this study was designed and analyzed using frequentist methodology, we
will now examine what the power and sample size calculations might look like if
the planned analysis were performed using Bayes factors. For doing so, we assume
a point analysis prior for the Bayes factor that equals the alternative
hypothesis specified by the trial investigators ($\mu = 1$ and $\tau = 0$), so
that the Bayes factor corresponds with a likelihood ratio. Hence, we can use the
power function~\eqref{eq:prLR} and sample size formula~\eqref{eq:nLR} to compute
power and sample size under point and normal design priors, which we illustrate
in the following.
...
latter shows a maximum Monte Carlo error of \Sexpr{round(sqrt(0.5*(1 -
  0.5)/nMC), 3)}. Depending on the ethical and financial stakes, this may or may
not be negligible for practical purposes, as the required sample size may be
slightly over- or underestimated. For example, in animal studies, under- or
overestimating the sample size by even one animal can be highly problematic and
a higher number of simulations should be performed.
...
Appendix~\ref{app:simeval} shows a more systematic comparison of the closed-form
and root-finding methods with Monte Carlo simulation-based methods, including
many other design and analysis priors. In summary, across the full range of
conditions studied, the sample sizes calculated by the closed-form and
root-finding methods produce the desired target power (estimated with
simulation) when the data were simulated with that sample size.
...
distributions. While there are differences between point and normal analysis
priors in terms of the closed-form availability of sample sizes (see
...
Section~\ref{sec:extensions} to more flexible design prior distributions.
Nevertheless, our work could still be further extended to ANOVA or regression
settings with multivariate parameters.
...
\texttt{bfpwr} R package. Values of the design and analysis prior means were
chosen to represent conventions for no (0), small (0.2), medium (0.5), and large
(0.8) standardized mean differences \citep{Cohen1992}. The null and alternative
hypotheses were defined as $H_{0}\colon \theta = 0$ against
...
computed (shown at the top of each plot). This sample size was subsequently used
to simulate \Sexpr{format(nsim, big.mark = "'")} standardized mean difference
parameter estimates based on which \Sexpr{format(nsim, big.mark = "'")} Bayes
factor were computed. The power was then estimated from the proportion of Bayes
factors equal or below the level $k = 1/\Sexpr{1/k}$. Note that for certain
design/analysis prior combinations, it is impossible to achieve the target power
with a finite sample size. In this case an ``x'' is shown in the plot.
...
We can see that in all conditions, the simulated-based estimate of the power is
close to the target power of \Sexpr{round(100*pow, 2)}\%. The maximally observed
discrepancy is \Sexpr{round(100*max(abs(res[,"power_MC"] - pow), na.rm = TRUE),
  2)}\% while the median discrepancy is
