...
\date{Epidemiology, Biostatistics and Prevention Institute (EBPI) \\
  ...
  {\color{blue} Preprint version June 19, 2024}}
...
% ======================================================================
\begin{abstract}
  \noindent Determining an appropriate sample size is a crucial element of study design, and it
  should be consistent with the planned analysis. If the planned analysis involves Bayes factor hypothesis testing, the sample size is usually desired to ensure a sufficiently high probability of obtaining a Bayes factor that indicates compelling evidence for a hypothesis, given that the hypothesis is true. In practice, Bayes factor sample size determination is typically performed using computationally intensive Monte Carlo simulation. Here we summarize an alternative approach that enables sample size determination without simulation. We show how, under approximate normality assumptions, sample sizes can be determined numerically, and provide the R package \texttt{bfpwr} for doing so. Additionally, we identify conditions under which sample sizes can even be determined in closed-form, resulting in novel, easy-to-use formulas that also help fostering intuition and enable asymptotic analysis. Furthermore, we show how in our framework, power and sample size can be computed without simulation for more advanced analysis priors, such as Jeffreys-Zellner-Siow priors or nonlocal normal moment priors. Case studies from medicine and psychology illustrate how researchers can use our methods for planning informative yet cost-efficient studies.
  \\
...
contrast, normal analysis and design prior can account for parameter
...
results. Another aim of our paper is to summarize sample size determination for
Bayes factor analysis based on numerical root-finding \citep{Weiss1997}, which,
while not available in closed-form, is much faster and deterministic compared to
...
quadratic equation in the sample size $n$. Its solution can be
% \begin{align*}
...
\end{align}
where $\Delta_{\mu_{d}} = 2\mu_{d} - \mu - \theta_{0}$, which reduces to the
ordinary `effect size' $\Delta_{\mu} = \mu - \theta_{0}$ when the design prior mean is
set to the parameter value under the alternative ($\mu_{d} = \mu$).
% \begin{align}
% \label{eq:nLR}
%     n = \frac{-b + \sqrt{b^{2} - 4ac}}{2a}.
% \end{align}
Of note, if power values $1 - \beta > 50\%$ are possible, replacing the first
plus in~\eqref{eq:nLR} by a minus gives the sample size that leads to a power of
$\beta$ instead of $1 - \beta$. The sample size~\eqref{eq:nLR} also typically
needs to be rounded to the next larger integer in order to be an evaluable
...
\begin{align}
  \label{eq:nLR2}
  n = \frac{\left[z_{1 - \beta} + \sqrt{z_{1 - \beta}^2 - \log k^2 (\Delta_{\mu_{d}}/\Delta_{\mu})}\right]^2}{\Delta_{\mu_{d}}^2 / \sigma^2_{\scriptscriptstyle \hat{\theta}}}.
\end{align}
...
\begin{align}
\label{eq:nLR3}
    n = \frac{\left\{z_{1 - \beta} + \sqrt{z_{1 - \beta}^2 - \log k^2}\right\}^2}{\Delta_{\mu}^2 / \sigma^2_{\scriptscriptstyle \hat{\theta}}}.
\end{align}
...
e.g.,][]{Edwards1971, Royall1997, Blume2002, Strug2018} -- are equivalent when
the Bayes factor involves only point hypotheses. Our more general
...
data models or prior distributions in the analysis. In the following, we outline
two extensions that modify the assumed Bayes factor in the analysis, while still
retaining the normal likelihood and point/normal design prior assumed in the
design.
...
\begin{enumerate}
  ...
  \item For a given design prior, compute the probability of obtaining a
        ...
        \mbox{$t \mid n, \mu_{d}, \tau^{2}_{d} \sim \mathrm{N}\{\mu_{d}\sqrt{n/2}, 1 + (n\tau^{2})/2\}$},
...
JZS Bayes factor \citep{Wetzels2009}, as in the original analysis. Using the
previously described iterative approach, we compute that a sample size of
...
implemented in our package (function \texttt{powertbf01}), takes about 100
milliseconds, while the simulation based approach takes about 10 minutes.
...
function~\eqref{eq:pnlBF}, numerical root-finding can be used to determine the
sample size such that a certain target power is achieved. Methods for power and
sample size calculations for normal moment prior Bayes factors are both
...
package \texttt{bfpwr}. We also derived novel sample size formulas that are
easy-to-use, help fostering intuition, and enable understanding of theoretical
properties such as asymptotic power or (non-)existence of samples size for a
given power. Compared to commonly used simulation-based methods, our methods are
less general. However, in the setting where they are applicable -- which
includes many common scenarios, such as testing mean differences -- they are
faster, deterministic, and require no simulation parameters to be specified.
