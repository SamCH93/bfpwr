...
\date{Epidemiology, Biostatistics and Prevention Institute (EBPI) \\
  ...
  {\color{blue} Working paper version May 14, 2024}}
...
\begin{abstract}
  ...
  testing, the sample size is typically desired to ensure a sufficiently high
  ...
  asymptotic analysis. Case studies from medicine and psychology illustrate how
  researchers can use
  our methods for planning informative but cost-efficient studies. \\
...
studies) or inefficient because the samples could be of better use in other
studies. Whether or not a certain sample size can ensure sufficiently conclusive
results depends on the planned analysis. Therefore, the sample size calculation
should be aligned with the planned analysis \citep{Anderson2022}, or in other
words: `\emph{As ye shall analyse is as ye shall design}'
\citep[p.~179]{Julious2023}.
...
of~\eqref{eq:freqsamplesize} that adapt it to unequal randomization, special
study designs (e.g., cross-over studies), or other data types (e.g., binary
data), see, for example, \citet{Kieser2020} or \citet{Julious2023}. For many
analysis methods, however, no closed-form formula exist and iterative or
simulation methods have to be used. Nevertheless, while typically only being an
approximation, the formula~\eqref{eq:freqsamplesize} allows for quick
...
data-based updating factor of the prior to posterior odds of two hypothesis.
Bayes factor approaches were pioneered by \citet{Jeffreys1939} and are now in
use in various scientific domains such as medicine \citep{Goodman1999},
psychology \citep{Wagenmakers2007}, or physics \citep{Trotta2008}. Bayes factor
tests are conceptually different from frequentist tests in several ways. For
example, they allow us to quantify evidence in favor of a null hypothesis or
they can incorporate external information via a prior distribution \citep[for an
overview of Bayes factors see e.g.,][]{Kass1995, Held2018}.
...
factors \citep{Weiss1997, DeSantis2004, Schoenbrodt2017, Schnbrodt2017b,
  Pawel2023e, Stefan2022}. However, to our knowledge, there are no simple
formulae such as~\eqref{eq:freqsamplesize} for sample size determination based
on Bayes factor analyses. In practice, sample size determination is often
performed by Monte Carlo simulation \citep{Gelfand2002, Schoenbrodt2017,
  Stefan2022}, but this can be inaccurate, time-consuming, and less intuitive
than a formula.
...
sample sizes are available. We also summarize sample size determination for
Bayes factors analysis based on numerical root-finding \citep{Weiss1997} that,
while not being available in closed-form, is much faster and deterministic
compared to simulation-based approaches. To facilitate reuse of our results, all
methods are made available through the R package \texttt{bfpwr}.
...
by deriving its distribution when new data are sampled
(Section~\ref{sec:distBF}). Combining these results, we derive several formulae
for the sample size under different constellations of design and analysis priors
(Section~\ref{sec:ssd}). Examples from medicine and the social sciences then
illustrates how our formulae can be implemented by researchers in practice
(Section~\ref{sec:application}). The paper ends with a closing discussion of
these results and final remarks on limitations and extensions
...
% -- Can we derive a closed-form formula that allows us to conduct sample size determination for Bayes factor analyses? (in case we are `stranded on a desert island' and need to do a quick-and-dirty sample size calculation to continue our research)
...
replacing $2\sigma^{2}$ with other unit variances from Table~\ref{tab:outcomes}.
It is important to note that these are only approximations and they may be
inadequate in certain situations, such as small sample sizes or rare events
\citep{Spiegelhalter2004}.
...
% of the effect estimate, for example, obtained from an estimated regression
...
\begin{table}[!htb]
  ...
  \caption{Different types of parameter estimates $\hat{\theta}$ with
    ...
    variance of one continuous outcome sample is denoted by $\sigma^{2}$ and
    parameter estimates based on two groups assume equal number of samples per
    group.}
  \label{tab:outcomes}
...
Assume now a point null hypothesis that states that $\theta$ equals a certain
null value $H_0 \colon \theta = \theta_0$ and the alternative hypothesis that
$\theta$ does not equal the null value $H_1 \colon \theta \neq \theta_0$, with
prior $\theta \mid H_1 \sim \mathrm{N}(\mu, \tau^2)$ assigned to $\theta$ under
$H_1$. The mean of the prior $\mu$ determines the most plausible parameter value
under the alternative while the variance determines its uncertainty. A point
alternative at $\mu$ can be obtained by setting the variance of the prior to
zero. % Here and henceforth, we define that whenever $\tau^{2} = 0$, a normal
% prior is equal to a point mass at its mean $\mu$ since for all calculations in
% this paper.
% , although technically this should be defined via a limit, yet it does not
% matter for the calculations in this paper).
...
The Bayes factor~\eqref{eq:BF01} has in some form or another appeared before in
the literature \citep[e.g.,][]{Weiss1997, DeSantis2004, Spiegelhalter2004}. It
may be thought of as a `Bayesian \mbox{$z$-test}', that is, a test of a normal
mean based on an asymptotically normal statistic assuming that the variance of
the statistic is known. Of course, the latter assumption is in most applications
not true, yet it makes the test widely applicable and is, for practical
purposes, often close enough to Bayes factors based on the exact distribution of
the data which may or may not be available. Furthermore, as we will show, it
also allows us to conduct closed-form power and sample size calculations.
...
\citet{Jeffreys1961} for `substantial' and `strong' evidence, respectively. To
...
normal alternative~\eqref{eq:BF01} can be derived in closed-form
...
Bayes factor literature. For the Bayes factor with point alternative
($\tau^2 = 0$), the cumulative distribution or `power function' is
\begin{align}
\label{eq:prLR}
...
% under null $\theta_{0}$).
However, with the Bayes factor based on a point alternative, depending on the
...
when the design prior is a point prior ($\tau^{2}_{d} = 0$), $Z_{\mathrm{lim}}$
diverges and the limitting power approaches one or zero, depending on whether
the location of the design prior $\mu_{d}$ is closer to the alternative $\mu$ or
to the null $\theta_{0}$. In case it is just in the middle of the two, the
limiting power approaches a half. On the other hand, for a normal design prior
($\tau^{2}_{d} > 0$), the limitting power is bounded by~\eqref{eq:powerlimLR}.

For the Bayes factor with normal prior under the alternative ($\tau^2 > 0$), the
...
based on the Bayes factor with point alternative~\eqref{eq:prLR}, the power
function based on the Bayes factor with normal alternative~\eqref{eq:prBF} can
...
to the point null hypothesis itself.
...
size $n$, or of other parameters. We provide R implementations of both in our
...
specified design prior. This is also implemented in our R package.
...
Bayes factors with point alternatives ($\tau^2 = 0$) and normal alternatives
($\tau^2 > 0$), we start again with the former.

\subsection{Bayes factor with point alternative}
% % Suppose that we are interested in the Bayes factor with point alternative
% % ($\tau^{2}$) ,
% Assume a normal design prior $\theta \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d})$.
% This prior may be set equal to the point alternative used in the analysis
...
e.g.,][]{Royall1997, Blume2002, Strug2018, Perneger2021} -- coincide when the
Bayes factor involves only point hypotheses. Our more general
formula~\eqref{eq:nLR} thus also generalizes evidential sample size calculations
to incorporate parameter uncertainty, similar to how Bayesian design priors can
be used for incorporating parameter uncertainty in a planned frequentist
analysis \citep[see e.g.,][for an overview of Bayesian/frequentist hybrid design
approaches]{Grieve2022}.

To illustrate this formula, we now assume that $\hat{\theta}$ is a standardized
mean difference with unit variance
...
size~\eqref{eq:nLR3} based on a standardized mean difference of one. We see, for
instance, that $n = 20$ samples per group are required to achieve
$1 - \beta = 80\%$ power for a Bayes factor threshold $k = 1/10$. If the assumed
standardized mean difference were smaller, the required sample size would become
larger. For example, for a half as large standardized mean difference, the
sample sizes from Table~\ref{tab:smdn} quadruple, e.g., requiring $n = 80$
samples per group to have $1 - \beta = 80\%$ power for a Bayes factor threshold
of $k = 1/10$.
...
\caption{Sample size per group $n$ to obtain a Bayes factor $\mathrm{BF}_{01}$
  smaller than $k$ with at least a power of $1 - \beta$ assuming a standardized
  mean difference of one under the alternative and for the design.}
\label{tab:smdn}
...
\subsection{Bayes factor with normal alternative}
Finding a sample size formula becomes more difficult when we move from point to
normal alternatives. The technical reason is that when we set the power
...
it is therefore sometimes also called `product logarithm'.

Suppose now that the design prior and the alternative are both centered around
the null value ($\mu_{d} = \mu = \theta_0$). Centering the prior around the null
value is commonly done in `default' Bayes factor tests \citep{Berger1987b}. It
encodes the assumption that some effect sizes are larger while others are
smaller than the null, the spread of the distribution determining the
variability, yet, on average, effects are equal to the null value. We then have
that $M=0$ and hence the power~\eqref{eq:prBF} reduces to
\begin{align}
  \label{eq:prBFcenter}
...
variance of the normal prior under the alternative ($\tau^2_{d} = \tau^{2}$). We
then have that
\begin{align*}
...
We can see that the required sample size $n$ depends on the ratio of the unit
variance $\sigma^{2}_{\scriptscriptstyle \hat{\theta}}$ to the prior variance
$\tau^{2}$, as well as a factor that we term `unit information sample size'
$n_{k,\beta}$ which depends only on the Bayes factor threshold $k$ and the
target power $1 - \beta$. As in classical sample size calculations, smaller unit
variances $\sigma^{2}_{\scriptscriptstyle \hat{\theta}}$ reduce the sample size.
The prior variance $\tau^{2}$ determines how large parameters are expected under
the alternative, and as such, larger prior variances lead to a reduction of
sample size similar to the effect size in classical sample size determination.
...
($\tau^{2} = \sigma^{2}_{\scriptscriptstyle \hat{\theta}}$). Finally, the
formula~\eqref{eq:nBF} allows us to study the potential existence of sample size
that can achieve the target power: Since the argument of the Lambert W function
has to be at least $-1/e$ for it to be defined, we can infer that only
combinations of Bayes factor thresholds $k$ and power values $1 - \beta$ that
satisfy $k^{2} z^{2}_{(1 - \beta)/2} \leq 1/e$ can actually be achievable with a
finite sample size. For example, it is impossible to find a sample size that
guarantees a power of $1 - \beta = 50\%$ for a threshold of $k = 1$ since then
...
calculations encode vastly different assumptions: The normal alternative
represents an effect size distribution that is centered around the null value
while the point alternative from~\ref{tab:smdn} represents a mean difference of
one standard deviation away from the null. The former is much more pessimistic
than the latter. To incorporate more optimistic beliefs into the calculations we
may increase the standard deviation of the distribution $\tau$ as this encodes
the assumption of potentially larger effect sizes. For example, doubling the
standard deviation $\tau$ leads to a four-fold decrease of the sample
size~\eqref{eq:nBF}.

The formula~\eqref{eq:nBF} is, to our knowledge, the first closed-form sample
size formula for Bayes factor analysis with normal alternatives. While
interesting from a technical point of view, its practical use is perhaps more
limited than the sample size formula for Bayes factors with point
alternatives~\eqref{eq:nLR}. This is because it makes the restrictive assumption
that the design prior and the analysis prior are both centered around the null
($\mu_{d} = \mu = \theta_{0}$). This seems unrealistic in practice, since
researchers designing a study usually have good reasons to expect effect sizes
to be different from the null and would like to account for this in the sample
size calculation. However, we have not been able to derive a sample size formula
for this more general setting due to the transcendental nature of the power
equation, even in terms of the Lambert W function. Fortunately, the sample size
can still be easily calculated numerically with our R package which is quicker
and more reliable than computing it with a simulation approach.
...
investigators, and the effect size of interest was the mean difference in time
to relief between the treatment and placebo groups $\theta$. The null hypothesis
was defined as no difference in efficacy of the treatment
($H_{0} \colon \theta = 0$), whereas the alternative was defined as a clinically
relevant difference of one day ($H_{1} \colon \theta = 1$). For sample size
calculations, the investigators used a data standard deviation of
...
alternative for the Bayes factor that equals the alternative specified by the
trial investigators ($\mu = 1$ and $\tau = 0$), so that the Bayes factor
corresponds with a likelihood ratio. Hence, we can use the power
...
\caption{Power and sample size calculations for MIST trial \citep{MIST1998}. The
  ...
  the null hypothesis $\mu_{d} = \theta_{0} = \Sexpr{null}$.}
\label{fig:MIST}
\end{figure}
...
used in the design as for the analysis. This number increases to
$n = \Sexpr{nanalyt2}$ when we move to a design prior that incorporates
parameter uncertainty (blue curve), i.e., a prior that is still centered around
$\mu = \Sexpr{pm}$ but with a standard deviation of $\tau_{d} = \Sexpr{dpsd2}$.
Finally, if we look at the orange power curve computed assuming that the null
hypothesis is true ($\theta = \Sexpr{null}$), we can see that the probability of
misleading evidence for the alternative when the null hypothesis is actually
true is very low and appears to be adequately controlled by conventional
standards (i.e., below 5\%) for each of the two sample sizes.
...
simulate data sets under an assumed design prior, and then analyze each data set
with a specified Bayes factor. This results in a distribution of Bayes factors
from which power and sample size can be calculated. This approach can be used in
quite general settings, but it is also computationally intensive and comes with
Monte Carlo error.
...
is not ($H_{1} \colon \theta \neq \Sexpr{null}$). We assume a
$\theta \mid H_{1} \sim \mathrm{N}(\Sexpr{pm}, 1/\Sexpr{1/psd^2})$ analysis
prior for the standardized mean difference under the alternative, as opposed to
the Cauchy prior with scale $1/\surd{\Sexpr{1/psd^2}}$ that was assumed by
\citet{Schoenbrodt2017}. As they did, we also study the two situations of a
point prior at $\theta = \Sexpr{dpm}$ for the design prior, which is a
convention for a `medium` standardized mean difference \citep{Cohen1992}, or a
normal prior at $\mu_{d} = \Sexpr{dpm}$ with standard deviation
...
We see that closed-form and simulation curves closely align in all cases, though
the former shows Monte Carlo error. Looking at the green curves in the top plot,
we see that a sample size of $n = \Sexpr{n}$ per group is sufficient to achieve
a target power of $\Sexpr{100*power}\%$ with a Bayes factor threshold of
$k = 1/\Sexpr{1/k}$ under the point design prior. This sample size is slightly
larger than the $n = 146$ that \citet{Schoenbrodt2017} obtained in their
calculations, which instead assumed a Cauchy analysis prior. As in the previous
example, incorporating parameter uncertainty via a normal design prior increases
the required sample size, in this case to $n = \Sexpr{n2}$. Looking at the
orange curve in the bottom plot, we see that these same sample sizes only have
...
power of $80\%$, say, around $n = 800$ samples per group would be required. This
illustrates the well-known fact that evidence accumulates slower under point
null hypotheses compared to normal alternatives when the Bayes factor involves
testing a point null against a normal alternative \citep{Johnson2010}. Finally,
...
researchers, in which case we believe it is better to do an approximate
calculation than no calculation at all. One avenue for future work might be to
extend closed-form sample size calculation to more specific settings, such as,
binary outcomes or continuous outcomes with unknown variances. Another
limitation is the type of Bayes factor that we considered for the analysis,
which is limited to univariate parameters with normal or point priors under the
alternative. Our work could be extended to ANOVA or regression settings with
multivariate parameters and/or to other prior distributions such as truncated
normal distributions to incorporate directionality. Finally, we did not consider
`open-ended' sequential designs, where data are collected continuously until
compelling evidence for one of the competing hypotheses is found
\citep{Wald1947}. The sequential approach is particularly attractive for Bayes
factor inference as design and analysis prior distributions can be naturally
updated based on the accumulating data. Researchers can then make informed
decisions about whether or not it is worthwhile to continue collecting data or
to stop \citep{Stefan2022}. For these purposes, it would be interesting to
consider the Bayes factor indexed by the sample size as a stochastic process,
...
can be used for any approximately normally distributed effect estimate with
approximate variance
$\mathrm{Var}(\hat{\theta}) = \sigma^{2}_{\scriptscriptstyle \hat{\theta}}/n$,
though users have to specify the unit variance themselves. The following code
...
\section{Limiting power of Bayes factor with normal alternative}
