...
\begin{abstract}
  ...
  sample size determination is typically performed using computationally
  intensive Monte Carlo simulation. Here we summarize an alternative approach
  that enables sample size determination without simulation. We show how, under
  approximate normality assumptions, sample sizes can be determined numerically,
  and provide the R package \texttt{bfpwr} for doing so. Additionally, we
  identify conditions under which sample sizes can even be determined in
  closed-form, resulting in novel, easy-to-use formulas that also help fostering
  intuition and enable asymptotic analysis. Furthermore, we show how in our
  framework, power and sample size can be computed without simulation for more
  advanced analysis priors, such as Jeffreys-Zellner-Siow priors or nonlocal
  normal moment priors. Case studies from medicine and psychology illustrate how
...
results. In addition to our novel formulas, we also summarize sample size
...
normal moment priors and Jeffreys-Zellner-Siow priors. While not available in
closed-form, root-finding approaches are much faster and deterministic compared
to simulation-based approaches. To facilitate reuse of our results, all methods
are made available through the R package \texttt{bfpwr}.
...
and a mean difference parameter (second row in Table~\ref{tab:outcomes}). The
formula could be generalized to other settings by replacing $2\sigma^{2}$ in the
numerator with other unit variances from Table~\ref{tab:outcomes}, which would
then change the interpretation of $n$ and $\hat{\theta}$. It is important to
note that these are only approximations and they may be inadequate in certain
situations, such as small sample sizes or rare events \citep{Spiegelhalter2004}.
...
parallels similar results on bounds for Bayesian/frequentist hybrid power
...
noting that in practice, the sample size formula~\eqref{eq:nLR} will usually
produce non-integer values and hence needs to be rounded to the next larger
integer in order to be an evaluable sample size (i.e., an actual number of
...
extensions that modify the assumed Bayes factor in the analysis, while still
