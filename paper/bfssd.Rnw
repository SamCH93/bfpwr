...
numerical root-finding. This is substantially faster than the Monte Carlo
simulation approach. To illustrate this, we now revisit the example from
\citet{Schoenbrodt2019} which was already re-analyzed in
Section~\ref{sec:schoenbrodtexample} with a normal analysis prior. We now
specify a one-sided Cauchy prior with scale $\tau = 1/\sqrt{2}$ as in the
original analysis. To

Root-finding based power and sample size calculation is also implemented in our
package via the function \texttt{powertbf01}.
...
Priors with density
$\mathrm{NM}(\theta \mid \theta_{0}, \tau) = \mathrm{N}(\theta \mid \theta_{0}, \tau) \times (\theta - \theta_{0})^{2}/\tau^{2}$
...
Under a normal design prior $\theta \sim \mathrm{N}(\mu_{d}, \tau^{2}_{d})$, the
power to obtain a Bayes factor below a threshold $k$ can (after some tedious
calculations) be expressed in closed-form
\begin{align}
  \label{eq:pnlBF}
  \Pr\left(\mathrm{BF}_{01} \leq k \mid n\right) =
