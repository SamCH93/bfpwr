...
\subsection{Randomized controlled clinical trial on locoregional radiotherapy}
\citet{You2020} conducted a randomized controlled clinical trial to assess the
efficacy of locoregional radiotherapy plus chemotherapy versus chemotherapy
alone in patients with nasopharyngeal carcinoma. The primary endpoint of the
trial was overall survival and the treatment effect was quantified with a log
hazard ratio $\theta$. For this type of parameter, the frequentist
sample size formula~\eqref{eq:freqsamplesize} can be adapted to
\begin{align}
  \label{eq:survival}
  n = \frac{4(z_{1 - \alpha/2} + z_{1 - \beta})^{2}}{(\mu - \theta_{0})^{2}}
\end{align}
which is obtained from taking the unit variance $\sigma^{2}_{\hat{\theta}} = 4$
and interpreting $n$ as the total number of events (i.e., deaths) required, see
Table~\ref{tab:outcomes}. The null hypothesis was defined as no difference in
the efficacy of the treatments ($H_{0} \colon \theta = 0$), while the
alternative hypothesis was formulated based on an estimated hazard ratio of
\Sexpr{exp(logHR)} from previous studies
($H_{1} \colon \theta = \log(\Sexpr{exp(logHR)}) = \Sexpr{round(logHR, 2)}$).
With a target power of $1 - \beta = \Sexpr{pow*100}\%$ and a two-sided level of
$\alpha = \Sexpr{alpha}$, formula~\eqref{eq:survival} gives a required number of
events of $n = \Sexpr{round(n, 1)}$. The investigators rounded this to
\Sexpr{round(n)}, although it might have been better to round to the next larger
integer \Sexpr{ceiling(n)} to ensure the desired target power. Based on
additional assumptions about the dropout and survival rates, the investigators
translated the required number of events into a total of 204 participants that
needed to be recruited (102 for each group). In the end, the trial fortunately
found a beneficial effect of the radiotherapy plus chemotherapy combination
compared to chemotherapy only (estimated hazard ratio of \Sexpr{exp(est)} with
95\% confidence interval from \Sexpr{exp(ci[1])} to \Sexpr{exp(ci[2])}).
...
look like if the planned analysis were performed using Bayes factors. For doing
so, we assume a point analysis prior that equals the alternative hypothesis
specified by the trial investigators ($\mu = \Sexpr{round(logHR, 2)}$ and
$\tau = 0$), so that the Bayes factor corresponds to a likelihood ratio. Based
on the estimated log hazard ratio $\hat{\theta} = \Sexpr{round(est, 2)}$ and its
standard error $\sigma = \Sexpr{round(se, 2)}$ (recalculated from the confidence
interval), we can apply equation~\eqref{eq:BF01} and obtain a Bayes factor
$\text{BF}_{01} = 1/\Sexpr{round(1/lr01, 1)}$, which indicates strong evidence
for the existence of a beneficial treatment effect over the null hypothesis of
no effect. Taking a step back and assuming that no data have yet been collected,
we can use equations~\eqref{eq:prLR} and \eqref{eq:nLR} to compute power and
sample size, which we illustrate in the following.
...
\caption{Power and sample size calculations for randomized clinical trial
  comparing efficacy of locoregional radiotherapy plus chemotherapy versus
  chemotherapy on overall survival in patients with nasopharyngeal carcinoma
  \citep{You2020}. The Bayes factor assumed for the analysis contrasts
...
Figure~\ref{fig:radiotherapy} shows power curves based on the Bayes factor providing
...
$n = \Sexpr{nanalyt}$ events are required to ensure a target power of
...
evidence for the alternative when the null hypothesis is actually true (the type
I error rate) is very low and appears to be adequately controlled by
conventional standards (i.e., below 5\%) for each of the two number of events.
This curve could be presented to a trial regulator to demonstrate that the
design is adequately calibrated.
