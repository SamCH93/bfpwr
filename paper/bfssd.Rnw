...
appropriate hypothesis \citep{Jeffreys1961, Held2018}.
...
emphasize again that one can and should also compute a power curve in favor of
the null hypothesis. That is, one can look at one minus the power
functions~\eqref{eq:prLR} and~\eqref{eq:prBF} along with a Bayes factor
threshold $k > 1$ and fixing the design prior to the null hypothesis
($\mu_{d} = \theta_{0}$, $\tau_{d} = 0$). In this way, sample sizes can be
determined that ensure a desired probability of compelling evidence for both the
...
\Sexpr{nanalyt}$ events are required to ensure a target power of $1 - \beta =
\Sexpr{round(power*100,2)}\%$ assuming that the same point prior is used in the
design as for the analysis (i.e., a point prior at $\mu = \Sexpr{pm}$). This
number increases to $n = \Sexpr{nanalyt2}$ when we move to a design prior that
incorporates parameter uncertainty (blue curve), i.e., a prior that is still
centered around $\mu = \Sexpr{pm}$ but with a standard deviation of $\tau_{d} =
\Sexpr{dpsd2}$. Finally, if we look at the orange power curve computed assuming
that the null hypothesis is true ($\theta = \Sexpr{null}$), we can see that the
probability of misleading evidence for the alternative when the null hypothesis
is actually true (the type I error rate) is very low and appears to be
adequately controlled by conventional standards (i.e., below 5\%) for each of
the two number of events. This curve could be presented to a trial regulator to
demonstrate that the design is adequately calibrated.

Focusing now on the bottom plot, we can see that the same number of events $n =
\Sexpr{nanalyt}$ also ensures a power of $1 - \beta =
...
Looking at the green curves in the top plot, we see that a sample size of
$n = \Sexpr{n}$ per group is sufficient to achieve a target power of
...
ensure a desired target power for a specified Bayes factor threshold. Data were
then simulated based on this sample size and the design prior. Bayes factors
were then calculated from the simulated data and the power empirically estimated
from the proportion of Bayes factors that were below the specified threshold.
These simulation-based power estimates were very close to the specified target
...
Nonlocal priors are prior distributions that have zero probability density/mass at
...
rather than by mathematical convenience. Point analysis priors seem more
appropriate when researchers have precise hypotheses to test, they can also be
motivated from a non-Bayesian likelihoodist perspective, and they are easier to
communicate to non-statistical audiences. In contrast, normal analysis priors
can incorporate uncertainty and may therefore be more appropriate for
exploratory settings where little is known about the phenomenon under
investigation \citep{Held2018}. Similar considerations apply to the choice of
the design prior. For example, a point design prior may be specified at a
...
$t$~likelihoods. Nevertheless, the approach could still be further extended to
...
should be careful not to give the impression that this choice has been made at a
priori.
