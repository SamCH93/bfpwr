...
can be used in some typical situations -- analysis and design based on the
\textit{z}-test Bayes factor (Section~\ref{sec:ztest}), the \textit{t}-test
Bayes factor (Section~\ref{sec:ttest}), and the normal moment prior Bayes factor
(Section~\ref{sec:nonlocal}). Table~\ref{tab:mainfunctions} summarizes the main
functions of the package. It is recommended to look at the function
...
also be used for `likelihoodist' analysis and design \citep[see e.g.,][for an
overview of the likelihoodist paradigm]{Royall1997}.
...
The following code demonstrates application to an estimated regression
...
association between the species Virginica and sepal width), over to the null
hypothesis of a log odds ratio equal to zero (i.e., no association). Note that
the Bayes factor from the function \texttt{bf01} and all other functions in
\textbf{bfpwr} are oriented \emph{in favor of the null} hypothesis over the
alternative, so $\mathrm{BF}_{01} > 1$ indicates evidence for the null
...
power (by specifying the \texttt{power} argument). The code below demonstrates
usage of \texttt{powerbf01} for a two-sample test of a standardized mean
...
To conduct such sample size and power calculations for other parameters than
means and (standardized) mean differences, additional assumptions regarding the
standard error and sample size are needed. The functions \texttt{pbf01} and
...
and setting its degrees of freedom to one, the Bayes factor reduces to the
...
\begin{figure}[!htb]
<< "prior-illustration", fig.height = 4.5, echo = FALSE >>=
tprior <- function(d,
                   plocation = 0,
                   pscale = 1,
                   pdf = 1,
...
that the future data that will be analyzed with a two-sample \textit{t}-test
Bayes factor using a directional JSZ analysis prior with a scale of
$1/\sqrt{2}$, and that a Bayes factor below \texttt{k = 1/6} is interpreted as
compelling evidence. In addition, a standardized mean difference of
\Sexpr{tres[["dpm"]]} was assumed for the underlying effect size, which
corresponds to setting the design prior mean to \texttt{dpm =
  \Sexpr{tres[["dpm"]]}} and its standard deviation to zero (\texttt{dpsd =
  \Sexpr{tres[["dpsd"]]}}). If parameter uncertainty is to be accounted for, one
could alternatively set a positive value for the design prior standard
deviation. Taken together, the calculation results in a sample size of
$n = \Sexpr{round(tres[["n"]], 2)}$, which means that at least
\Sexpr{ceiling(tres[["n"]])} observations per group are required to obtain
...
applicable to data in the form of parameter estimates with standard errors. The
difference between the two approaches is that the former uses a so-called
`normal moment' prior distribution for the parameter under the alternative
hypothesis. Normal moment priors are a type of `non-local' prior distribution
which allow evidence for a point null hypothesis to accumulate more quickly if
it is indeed true \citep{Johnson2010}. This is because normal moment priors have
no density at the null value, see Figure~\ref{fig:nmprior} for an illustration.

\begin{figure}[!htb]
<< "nm-prior-illustration", fig.height = 4.5, echo = FALSE >>=
...
functions that can accommodate also other parameter types (\texttt{pnmbf01} and
